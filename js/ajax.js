...
function sign() {
    $("#sign").html("主人，请稍等正在签到...");
    xiha.postData("api.php?do=sign", "r=" + Math.random(1), function(d) {
        if (d.code == 200) {
            $("#sign").html("主人，签到成功，经验+" + d.point);
        } else { $("#sign").html("签到失败咯~:" + d.msg); }
    });
}
...
function daka() {
    ...
    xiha.postData("api.php?do=daka", "r=" + Math.random(1), function(d) {
        if (d.code == 200) {
            $("#daka").html("好棒，打卡成功，听歌" + d.count + "首");
        } else { $("#daka").html("未知错误，请联系QQ3113760772"); }
    });
}
